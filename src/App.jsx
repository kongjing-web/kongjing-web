        ...
        <div className="relative" ref={menuRef}>
          ...
          {showUserMenu && (
            <div className="absolute left-0 mt-2 w-44 bg-white rounded-2xl shadow-xl border border-gray-100 p-1.5 z-50">
              ...
              {/* 👑 新增：如果当前账号是管理员，在菜单内无缝追加后台入口 */}
              {isAdmin && (
                <button 
                  onClick={() => { onNavigateAdmin(); setShowUserMenu(false); }} 
                  className="w-full flex items-center gap-2.5 px-3 py-2 text-left text-xs font-semibold text-amber-700 bg-amber-50/50 hover:bg-amber-50 rounded-xl transition-colors"
                >
                  <FaLock className="text-amber-600" /> {t('home_admin_panel') || '管理员后台'}
                </button>
              )}              
